...
- 1 当页面打开时还未完成加载，YeeUI 会先给  `<html>` 添加css样式 pointer-events:none 禁用点击事件 ，并且等到页面加载完成。  
...
-  4 渲染完成后 移除 `<html>` css样式 pointer-events:none 并执行 jQuery 的 $(function(){}); ready 方法。   此处使用 jQuery 的 ready 方法会被延后到 所有节点渲染完成。    
