class YeeUpload {
    ...
    upImgGroup() {
        ...
        let useName = qel.data('use-name') || false;
        ...
        const update = function () {
            ...
            shower.find('div.img-item').each(function (index, element) {
                ...
                if (useName) {
                    ...
                    imgItems.push({url: url, name: name});
                } else {
                    imgItems.push(url);
                }
            });
        ...
        let addImg = function (url, name) {
            if (size > 0 && shower.find('div.img-item').length >= size) {
                return;
            }
            let item = $('<div class="img-item"></div>');
            item.data('value', url);
            item.data('name', name);
        ...
        let initValue = function () {
            ...
            for (let i = 0; i < arrValue.length; i++) {
                if (typeof arrValue[i] == 'string') {
                    addImg(arrValue[i], url2Name(arrValue[i]));
                } else {
                    addImg(arrValue[i].url, arrValue[i].name);
                }
            }
        };
        initValue();
                ...
                if (ret.data) {
                    if (ret.data.files && ret.data.files.length > 1) {
                        for (let item of ret.data.files) {
                            if (item.url) {
                                addImg(item.url, item.orgName ?? '');
                            }
                        }
                    } else if (ret.data.url) {
                        addImg(ret.data.url, ret.data.orgName ?? '');
                    }
                }
                if (ret.msg) {
                    Yee.msg(ret.msg);
