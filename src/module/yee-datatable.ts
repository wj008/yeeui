...
export class YeeDatatable {
    ...
    public init(data = null, url = null) {
        this.initColumn();
        this.initLeftFixed();
        this.initRightFixed();
        this.initEvent();
     ...
     * @param param
     */
    public load(url: string = null, param: { [p: string]: any } = {}, showMessage = false) {
        ...
        $.ajax({
            ...
            success: function (ret) {
                ...
                if (ret.status === true) {
                    ...
                    that.render(ret);
                    // @ts-ignore
                    qel.emit('success', ret, Yee.toUrl(args));
                    deferred.resolve(ret, Yee.toUrl(args));
                }
                //拉取数据错误
                if (ret.status === false) {
